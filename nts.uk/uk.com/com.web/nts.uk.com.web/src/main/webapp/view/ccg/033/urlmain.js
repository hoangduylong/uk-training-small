__viewContext.ready(function () {
    ...
    nts.uk.request.ajax("com", server_path).done(function (success) {
        ...
        if (!nts.uk.util.isNullOrUndefined(success.changePw.successMsg) && !nts.uk.util.isNullOrEmpty(success.changePw.successMsg)) {
            nts.uk.ui.dialog.info({ messageId: success.successMsg }).then(function () {
...
function routeData(success, urlID) {
    if (success.smpDevice && (success.programID == 'kaf005' || success.programID == 'cmm045')) { //Mobile
        var path_1 = "/view/kaf/000/b/index.xhtml" + '?programID=' + success.programID + '&appId=' + _.first(_.map(success.urlTaskValueList));
        if (success.programID == 'kaf005') {
            nts.uk.request.ajax("at", 'at/request/application/approval/judgmentuser', _.first(_.map(success.urlTaskValueList)))
                ...
                path_1 = path_1 + '&userAtr=' + result;
                nts.uk.request.jump("at", path_1);
            });
        }
        else {
            nts.uk.request.jump("at", path_1);
    ...
    switch (success.programID) {
        case "ccg007": {
            ...
            var urlID_1 = _.split(path, '=')[1];
            // forgot password screen
            nts.uk.request.jump("com", "/view/ccg/007/" + success.screenID + "/index.xhtml?id=" + urlID_1);
            break;
        }
        case "kaf002": {
        ...
        case "kmw003": {
            var normalMode = success.urlTaskValueList["activeMode"] == "normal" ? true : false;
            if (normalMode) {
...
//# sourceMappingURL=urlmain.js.map